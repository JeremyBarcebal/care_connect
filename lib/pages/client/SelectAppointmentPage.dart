...
const Color kPrimaryColor =
    Color(0xFF48A6A7); 
const Color kBookedSlotColor =
    Color(0xFFFDD8D8);
const Color kBookedBorderColor =
    Color(0xFFD93025); 
...
class _SelectAppointmentPageState extends State<SelectAppointmentPage> {
  ...
  void _loadMockInitialData() {
    // =========================================================================
    // 🔥 FIREBASE CONNECTION POINT 1: Initial Data Load (Mocked for now)
    //    Replace this mock logic with a real database call to fetch the
    //    default available and booked slots for today's date.
    // =========================================================================
    setState(() {
      availableTimes = [
        "12:00 PM",
      ...
      // Mock one time slot as already booked today
      bookedTimes = ["1:30 PM"];
    });
  }
  ...
  Future<void> fetchAvailableTimes(DateTime date) async {
    // =========================================================================
    // 🔥 FIREBASE CONNECTION POINT 2: Fetch Slots for Selected Date
    //    This is where the logic needs to be added to query the database
    //    to get the available and booked times for the given `date` and 
    //    `widget.doctorId`.
    // =========================================================================
    ...
    if (date.day % 2 == 0) {
      ...
      // Third day multiples have no available slots
      fetchedAllTimes = [];
      fetchedBookedTimes = [];
    } else {
      // Other days (like 1, 5, 7, etc.)
      fetchedAllTimes = ["11:00 AM", "1:00 PM", "2:00 PM", "3:00 PM"];
      fetchedBookedTimes = ["2:00 PM"]; // Mocked booked slot
    }
    // --- MOCK DATA LOGIC END ---
            ...
            child: ElevatedButton(
              onPressed: selectedTime != null &&
                  ...
                  ? () {
                      ...
                      // Currently just pops back with the selected DateTime
                      Navigator.pop(context, appointmentDateTime);
                    }
                  : null,
